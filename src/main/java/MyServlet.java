	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("signup") != null) {
			...
			if (!password.equals(password_confirm)) {
				...
				//out.println("alert("+"'"+message+"'"+");window.location='./signup.jsp';</script>");
			...
			MyBank customer = new MyBank(firstName, lastName, phone, email,line1,line2,city,state,pincode, password,amount,type);
		...
		if(request.getParameter("deposit")!=null)
		{
			...
			{
				//out.println("<script type='stext/javascript'>");
				//out.println("alert("+"'"+result+"'"+");window.location='./transactions.jsp';</script>");
				//response.sendRedirect("./transactions.jsp");
		...
		if(request.getParameter("close")!=null)
		{
			...
			/*String url="transactions.jsp?id="+Customer_id;
			out.println("<script type='text/javascript'>");
			out.println("var con=confirm('Are you sure you want to close this account. Press Ok to confirm');"
					+ "if(con==false)"
					+ "window.location="+"'"+url+"'"+";"
					+ "</script>");*/
