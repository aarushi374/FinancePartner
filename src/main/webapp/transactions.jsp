...
<body>
	...
	<nav>
		...
		<button class="btn_home" onclick="logout()">
			<i class="fas fa-home"></i>Home
		</button>
	</nav>
	<p class="login_heading">
		Current Balance:
		<%=resultSet.getInt("amount")%></p>
	...
	<div id="address_change" class="transaction-form" style="display: none">
		<form action="MyServlet" method="post">
			...
			<p></p>
			<label class="transaction_text">Line 1</label><br> <input
				type="text" name="line1" class="input_login" required><br>
			<label class="transaction_text">Line 2</label><br> <input
				type="text" name="line2" class="input_login"><br> <label
				class="transaction_text">City</label><br> <input type="text"
				name="city" class="input_login" required><br> <label
				class="transaction_text">State</label><br> <input type="text"
				name="state" class="input_login" required><br> <label
				class="transaction_text">Pincode</label><br> <input
				type="number" min="100000" max="999999" name="pincode"
				class="input_login"
			...
			<button type="submit" name="new_address"
				value='<%=request.getParameter("id")%>' class="btn_form">Submit</button>
	...
	<div id="money_transfer" class="transaction-form" style="display: none">
		<form action="MyServlet" method="post">
			...
			<button type="submit" value='<%=request.getParameter("id")%>'
				name="transfer" class="btn_form">Submit</button>

		</form>
	</div>
	<button class="btn_transaction" onclick="openAddressForm()">Address
		Change</button>
	<button class="btn_transaction" onclick="openDepositForm()">Money
		Deposit</button>
	<button class="btn_transaction" onclick="openWithdrawalForm()">Money
		Withdrawal</button>
	<button class="btn_transaction" onclick="openTransferForm()">Transfer
		Money</button>
	<form action="MyServlet" method="post"
		style="margin: 0px; padding: 0px; display: inline;">
		<button class="btn_transaction" name="print"
			value='<%=request.getParameter("id")%>'>Print Statement</button>
	</form>
	<form method="post" action="MyServlet"
		style="margin: 0px; padding: 0px; display: inline;">
		<button class="btn_transaction" name="close"
			value='<%=request.getParameter("id")%>'>Account Closure</button>
	</form>
	<div class="footer">
		<div class="foot">
