...
<body>
	...
	<div class="">
		<p class="heading2">Account Statement</p>
		<div class="date">
			<form method="post" action="MyServlet">
				<label class="text2">Start date</label><input type="date"
					id="datePicker1" name="start_print"> <label class="text2">End
					date</label><input type="date" id="datePicker2" name="end_print">
				<button type="button" onclick="date()"
					value='<%=request.getParameter("id")%>' name="button" id="btn"
					class="btn_date">Submit</button>
		...
		try {
			...
			String sql = "select * from print where customer_id=? and date between ? and ? order by date desc";
		...
		<table>
			...
			<tr>
				...
				<td><%=resultSet.getInt("toaccount")%></td>
		...
		function date() {
			if (document.getElementById('datePicker1').value == ""
				...
				document.getElementById('text').innerHTML = x;
					...
					&& document.getElementById('datePicker2').value != "") {
				var customer_id = document.getElementById('btn').value;
				window.location = "accountStatement.jsp?id=" + customer_id
						+ "&start="
