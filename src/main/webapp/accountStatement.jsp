...
<div class="">
  ...
  <button type="button" onclick="date()" value='<%=request.getParameter("id")%>'name="button" id="btn" class="btn_date">Submit</button>
  <p class="text" id="text"></p>
  </form>
  </div>
  <%
  //System.out.print(request.getParameter("start_print"));
		try {
			Connection con = null;
...
function date()
{
  ...
  else if(document.getElementById('datePicker1').value!="" && document.getElementById('datePicker2').value!="")
  {
    ...
    window.location="accountStatement.jsp?id="+customer_id+"&start="+document.getElementById('datePicker1').value+
